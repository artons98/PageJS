...
window.PageJSStylesURL = 'https://cdn.jsdelivr.net/gh/artons98/PageJS/dist/1.0.34/pageJS.css';
    ...
    if (!PageJS.Utils.ContextMenu) {
        PageJS.Utils.ContextMenu = class {
            ...
            static _bindMenuInternalBlockers(menuEl){
                ...
                menuEl.addEventListener('click', stop, true);
                menuEl.addEventListener('mousedown', stop, true);
                menuEl.addEventListener('mouseup', stop, true);
                menuEl.addEventListener('pointerdown', stop, true);
                menuEl.addEventListener('touchstart', stop, true);
                // voorkom native contextmenu binnen ons menu en doorsijpelen
                menuEl.addEventListener('contextmenu', preventAndStop, true);
                menuEl._pagejsMenuBlockersBound = { stop, preventAndStop };
            }

            static _unbindMenuInternalBlockers(menuEl){
                const b = menuEl._pagejsMenuBlockersBound;
                if (!b) return;
                menuEl.removeEventListener('click', b.stop, true);
                menuEl.removeEventListener('mousedown', b.stop, true);
                menuEl.removeEventListener('mouseup', b.stop, true);
                menuEl.removeEventListener('pointerdown', b.stop, true);
                menuEl.removeEventListener('touchstart', b.stop, true);
                menuEl.removeEventListener('contextmenu', b.preventAndStop, true);
