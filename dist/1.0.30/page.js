...
window.PageJSStylesURL = 'https://cdn.jsdelivr.net/gh/artons98/PageJS/dist/1.0.30/pageJS.css';
            ...
            try{
                ...
                if (colors && (colors.primary || colors.secondary)) {
                    const toRgb = (val) => {
                        ...
                        // rgb/rgba formats
                        const m = String(val).match(/rgba?\s*\(\s*([0-9.]+)\s*,\s*([0-9.]+)\s*,\s*([0-9.]+)/i);
                    ...
                    let css = ':root{';
                    if (primary) css += `--bs-primary: ${primary};`;
                    if (primaryRgb) css += `--bs-primary-rgb: ${primaryRgb.join(', ')};`;
                    if (secondary) css += `--bs-secondary: ${secondary};`;
                    if (secondaryRgb) css += `--bs-secondary-rgb: ${secondaryRgb.join(', ')};`;
                    css += '}';
