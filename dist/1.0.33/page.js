...
window.PageJSStylesURL = 'https://cdn.jsdelivr.net/gh/artons98/PageJS/dist/1.0.33/pageJS.css';
...
(function(){
    ...
    if (!PageJS.Utils.ContextMenu) {
        PageJS.Utils.ContextMenu = class {
            ...
            /**
             * Basis callback voor wanneer geen data-context-options aanwezig is.
             * Return null/undefined om niets te tonen (rechtermuisklik genegeerd).
             * Kan later aangepast worden om standaard opties te genereren.
             * @param {Element} target
             * @param {MouseEvent} event
             * @returns {null|string|Array<{text:string, action?:Function, href?:string}>}
             */
            static BaseCallback(target, event){
                return null; // standaard: niets tonen => rechtermuisklik genegeerd
            }

            /**
             * Toont een contextmenu bij de cursorpositie.
             * Bronnen voor opties:
             * - data-context-options attribuut op target of ancestors
             *   - Waarde die start met '<' => raw HTML (ul/li of enkel li's)
             *   - Waarde die start met '#' => selector voor template-element
             *   - Anders: probeert functie op KO context of window aan te roepen die HTML of items-array retourneert
             * - Zo niet aanwezig: PageJS.Utils.ContextMenu.BaseCallback()
             * @param {MouseEvent} e
             */
            static ShowAtCursor(e){
                try{ e.preventDefault(); }catch{}

                const target = (e.target && e.target.closest) ? (e.target.closest('[data-context-options]') || e.target) : (e.target || document.body);
                const content = this._resolveOptions(target, e);

                if (!content) {
                    this.hide();
                    return; // niets tonen (rechtermuisklik genegeerd)
                }

                const ul = this._getOrCreateMenu();
                // Vul content
                if (typeof content === 'string') {
                    // Als volledige <ul> is meegegeven, pak de innerHTML
                    if (content.toLowerCase().includes('<ul')) {
                        const tmp = document.createElement('div');
                        tmp.innerHTML = content.trim();
                        const ulIn = tmp.querySelector('ul');
                        ul.innerHTML = ulIn ? ulIn.innerHTML : content;
                    } else {
                    ...
                    content.forEach(item => {
                        ...
                        if (typeof item.action === 'function') {
                            a.addEventListener('click', (ev) => {
                                ev.preventDefault();
                                try{ item.action(ev, target); }catch(err){ console.warn('[PageJS.ContextMenu] action error:', err); }
                                PageJS.Utils.ContextMenu.hide();
                            });
                        } else {
                            a.addEventListener('click', (ev) => {
                                // default: enkel sluiten
                                ev.preventDefault();
                                PageJS.Utils.ContextMenu.hide();
                            });
                        }
                ...
                // Bind KO context indien beschikbaar
                try{
                    if (window.ko && typeof ko.applyBindings === 'function'){
                        const vm = ko.dataFor(target) || null;
                        if (vm) ko.applyBindings(vm, ul);
                    }
                }catch{}

                // Positie berekenen (viewport coordinates)
                const x = (e.clientX ?? 0);
                const y = (e.clientY ?? 0);
                ul.style.left = `${x}px`;
                ul.style.top = `${y}px`;
                ul.style.visibility = 'hidden'; // eerst meten
                ul.classList.add('show');

                // Zorg dat in viewport blijft
            ...
            static _attachOutsideHandlers(){
                ...
                this._outsideHandlerBound = (ev) => {
                    ...
                    if (ev.type === 'click'){
                        if (!ul.contains(ev.target)) this.hide();
                    }
                };
            ...
            static _getOrCreateMenu(){
                if (this._menuEl) {
                    try{ this._menuEl.remove(); }catch{}
                }
                const ul = document.createElement('ul');
                ul.className = 'dropdown-menu show';
                ul.style.position = 'fixed';
                ul.style.margin = '0px';
                ul.style.zIndex = (PageJS?.UIController?.getHighestZIndex?.() || 10000) + 1;
                document.body.appendChild(ul);
                this._menuEl = ul;
                return ul;
            }

            static _resolveOptions(target, e){
                // Zoek dichtstbijzijnde element met data-context-options
                let el = target && target.closest ? target.closest('[data-context-options]') : null;
                let val = el ? el.getAttribute('data-context-options') : null;

                if (val && typeof val === 'string'){
                    const v = val.trim();
                    // Case 1: raw HTML
                    if (v.startsWith('<')) return v;
                    // Case 2: selector naar template
                    if (v.startsWith('#')){
                        const tpl = document.querySelector(v);
                        if (tpl){
                            if (tpl.tagName.toLowerCase() === 'template'){
                                return tpl.innerHTML.trim();
                            }
                            if (tpl.tagName.toLowerCase() === 'ul'){
                                return tpl.innerHTML.trim();
                            }
                            return tpl.innerHTML.trim();
                        }
                    }
                    // Case 3: probeer als functie op KO context of window
                    const fn = this._resolveFunction(v, el || target);
                    if (typeof fn === 'function'){
                        try{
                            return fn.call(this._resolveContext(el || target), el || target, e);
                        }catch(err){ console.warn('[PageJS.ContextMenu] Optie-functie faalde:', err); }
                    }
                    // Case 4: als JSON array van items
                    try{
                ...
                // Geen attribuut of geen bruikbare waarde => fallback
                try{ return PageJS.Utils.ContextMenu.BaseCallback(el || target, e); }catch{ return null; }
            }

            static _resolveFunction(path, el){
                const ctx = this._resolveContext(el);
            ...
            static _resolveContext(el){
                try{ return (window.ko && typeof ko.dataFor === 'function') ? (ko.dataFor(el) || window) : window; }catch{ return window; }
            }
