...
window.PageJSStylesURL = 'https://cdn.jsdelivr.net/gh/artons98/PageJS/dist/1.0.31/pageJS.css';
    ...
    PageJS.Styling = class{
        ...
        static applyColorsFromSettings(){
            try{
                ...
                const originalPrimaryValue = getVar('--bs-primary') || '';
                const originalSecondaryValue = getVar('--bs-secondary') || '';
                const originalPrimaryRgb = toRgbString(originalPrimaryValue);
                const originalSecondaryRgb = toRgbString(originalSecondaryValue);

                const primaryNew = colors.primary || null;
                const secondaryNew = colors.secondary || null;
                const primaryNewRgb = toRgbString(primaryNew);
                const secondaryNewRgb = toRgbString(secondaryNew);
                ...
                for (const sheet of Array.from(document.styleSheets)) {
                    ...
                    for (const rule of Array.from(rules)) {
                        if (rule.style) {
                            ...
                            for (const r2 of Array.from(rule.cssRules)) {
                                if (r2.style) collectFromStyle(r2.style);
                            }
                        }
                    }
                }
                ['--bs-primary','--bs-secondary','--bs-primary-rgb','--bs-secondary-rgb'].forEach(n => varNames.add(n));

                const refPrimaryRegex = /var\(\s*--bs-primary\s*\)/i;
                const refSecondaryRegex = /var\(\s*--bs-secondary\s*\)/i;
                ...
                varNames.forEach(name => {
                    ...
                    if (refPrimaryRegex.test(val)) {
                        primaryTargets.push(name);
                        return;
                    }
                    if (refSecondaryRegex.test(val)) {
                        secondaryTargets.push(name);
                        return;
                    }

                    const rgb = toRgbString(val);
                    if (originalPrimaryRgb && rgb === originalPrimaryRgb) {
                        primaryTargets.push(name);
                    } else if (originalSecondaryRgb && rgb === originalSecondaryRgb) {
                        secondaryTargets.push(name);
                    }
                });

                let css = ':root{';
                if (primaryNew && primaryTargets.length) {
                    for (const n of primaryTargets) {
                        if (n.endsWith('-rgb') && primaryNewRgb) css += `${n}: ${primaryNewRgb};`;
                        else css += `${n}: ${primaryNew};`;
                    }
                }
                if (secondaryNew && secondaryTargets.length) {
                    for (const n of secondaryTargets) {
                        if (n.endsWith('-rgb') && secondaryNewRgb) css += `${n}: ${secondaryNewRgb};`;
                        else css += `${n}: ${secondaryNew};`;
                    }
                }
                css += '}';
